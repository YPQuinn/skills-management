...
- [Implement Groups, Targets, and Assignments](issues/13-implement-groups-targets-assignments.md) — Delivered overlapping Groups, safe fixed-path built-in and custom Targets, advisory adapter detection, idempotent Skill and Group Assignments, explained desired-set expansion, replacement-impact previews, and matching CLI, REST, and bilingual Appica workflows.
- [Implement synchronization, diff, and rollback](issues/14-implement-synchronization-rollback.md) — Delivered coherent ten-state synchronization, three-way differences, explicit conflict actions, durable Store and Baseline recovery, safe batching, reversible rollback, and equivalent CLI, REST, and bilingual Appica workflows.
- [Implement Distribution and adoption](issues/15-implement-distribution-adoption.md) — Delivered safe pinned-handle reconciliation, Managed Link ownership, durable crash recovery, explicit adoption, coherent status and outcomes, and equivalent CLI, REST, and bilingual Appica workflows.
