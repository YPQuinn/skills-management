    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 text-sm">
        <div className="border border-border rounded-xl p-4 bg-background">
          <div className="text-foreground-muted">{t('labelSlug')}</div>
          <div className="font-mono font-medium mt-0.5">{skill.slug}</div>
        </div>
        <div className="border border-border rounded-xl p-4 bg-background">
          <div className="text-foreground-muted">{t('labelCreated')}</div>
          <div className="font-medium mt-0.5">{formatTime(skill.created_at)}</div>
        </div>
        <div className="border border-border rounded-xl p-4 bg-background">
          <div className="text-foreground-muted">{t('labelUpdated')}</div>
      ...
      <div className="border border-border rounded-xl p-6 bg-background space-y-4">
        ...
        {skill.binding ? (
          <div className="grid gap-4 sm:grid-cols-2 text-sm">
            <div>
              <div className="text-foreground-muted">{t('labelSourceName')}</div>
            ...
            <div>
              <div className="text-foreground-muted">{t('labelImportedAt')}</div>
          ...
          <p className="text-sm text-foreground-muted">{t('unboundSkillText')}</p>
        )}
      </div>
    </div>
  )
