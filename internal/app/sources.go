...
func (a *App) CheckSource(ctx context.Context, id int64) (*source.Source, error) {
	...
	// The Store-overlap guard is re-evaluated on every Local check: a
	// post-registration symlink swap of the Source root into the Store
	// must never be scanned, and the Source's physical identity is
	// verified by ResolveLocalSubpath inside the guard.
