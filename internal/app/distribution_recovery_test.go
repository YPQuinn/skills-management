...
// TestRecoveryPendingCreateExactSymlink proves a pending create whose
// exact expected symlink is at the final slug completes the ledger.
func TestRecoveryPendingCreateExactSymlink(t *testing.T) {
	a, targetID, _, tv := seedLinkFixture(t)
	importAllSkills(t, a, "other")
	otherID := skillIDBySlug(t, a, "other")
	assignSkill(t, a, targetID, otherID)
