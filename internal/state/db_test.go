...
func TestOpenAppliesAndReopensMigrations(t *testing.T) {
	...
	if got := appliedVersion(t, db); got != 10 {
		t.Fatalf("schema version after open: got %d, want 10", got)
	}
	db.Close()
	...
	if got := appliedVersion(t, db); got != 10 {
		t.Fatalf("schema version after reopen: got %d, want 10", got)
	}
	db.Close()
}
...
func TestInspectSchema(t *testing.T) {
	...
	if got, err := InspectSchema(path); err != nil || got != 10 {
