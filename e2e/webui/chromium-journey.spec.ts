...
test('Chromium WebUI core journey against embedded skillctl', async ({ page }) => {
  ...
  try {
    ...
    await chooseSelect(page, 'Group', 'crew')
    await page.getByRole('button', { name: 'Assign' }).click()
    await expect(page.getByRole('heading', { name: /Desired Skill Set/ })).toBeVisible()
    await expect(page.getByRole('table', { name: /Desired/ }).getByRole('link', { name: 'demo', exact: true })).toBeVisible()
    ...
    await expect(page.getByRole('alertdialog', { name: 'Distribution result' })).toBeVisible()
    await expect(page.getByRole('alertdialog').getByText('1 created')).toBeVisible()
