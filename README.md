...
```text
grep 'skillctl_v0.1.3_linux_amd64.tar.gz$' SHA256SUMS | sha256sum -c -
tar -xzf skillctl_v0.1.3_linux_amd64.tar.gz
mkdir -p ~/.local/bin
...
Maintainers run `./scripts/release.sh v0.1.3` from a clean checkout at that exact tag. It runs the quality and README gates, then produces four archives and `SHA256SUMS`.
